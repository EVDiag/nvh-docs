...
@media print {
  /* Layout: full page width, remove screen padding */
  body { max-width: none; padding: 0; color: #000; background: #fff; }

  /* Hide screen-only navigation and controls */
  .download-banner, .lang-switcher, .top-nav, .docs-footer,
  .print-button { display: none !important; }

  /* Disclaimer styled minimally for print (no colored background) */
  ...
  table { page-break-inside: avoid; border-collapse: collapse;
          width: 100%; }
  th, td { border: 1px solid #888; padding: 6pt 8pt; }
  th { background: #f0f0f0; color: #000; }

  /* Links: keep blue, show URL after external links so paper readers
     can find them. Same-page anchor links don't get URL appended. */
  a { color: #003366; text-decoration: underline; }
  a[href^="http"]::after { content: " (" attr(href) ")";
                            font-size: 9pt; color: #555;
                            word-break: break-all; }

  /* Code: keep monospace, light box */
  code { background: #f6f6f6; padding: 1pt 3pt; border-radius: 2pt;
         font-size: 10pt; }
  pre { background: #f6f6f6; padding: 8pt; page-break-inside: avoid;
        font-size: 9pt; border: 1px solid #ddd; }

  /* Suppress URL in TOC chapter links (in-page anchors) */
  .toc a::after, ol a::after, ul a::after { content: ""; }

  /* Comfortable text size on paper */
...
<body>
  ...
  <div class="download-banner">
    <span>{title}</span>
    <button class="print-button" onclick="window.print()">🖨️ {ui["print_pdf"]}</button>
